...
- **今日以前の日はタップ可能**（未来日はタップ不可）：タップでその日の中央モーダルを表示
  - 記録ありの日：日付見出し＋タイトル＋6項目のラベル付き表示（複数件は縦に並べる）＋ 末尾に「この日の日記を書く」ボタン（Primary Filled・コンパクト幅）
  - 記録なしの日：空状態の文言（"この日の記録はまだありません"、14px, secondary, 中央）＋「この日の日記を書く」ボタン
  - ボタンタップで `/record?date=YYYY-MM-DD`（過去日付モード）へ遷移。✕/背景タップで閉じる
