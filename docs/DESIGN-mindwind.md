...
- **音声入力の実装: Web Speech API**
  ...
  - 録音中のプライバシー注記: "音声はブラウザの音声認識サービスに送信されます"（Micro, 13px, `rgba(0,0,0,0.58)`）をテキストエリア下に表示
- **送信ボタン（テキストエリアの右下内側）:**
  ...
  - Loading state: アイコンを loader に差し替え + テキストエリア下に "分析中..."
...
- トースト通知（画面上部からスライドイン）:
  ...
  - Shadow: card shadow
- **記録タブへ戻ったとき**: 当日のチャットが保存済み状態で復元される。成形カードは読み取り専用＋「保存済み」バッジ（12px, `#d4e9e2` bg, `#006241` text)。入力バーからそのまま**2件目の日記**を書ける（同じフローで別の記録として保存）
- **1日の上限は3件**: 3件記録済みの日は入力バーを無効化（マイク・送信ボタンを tertiary 色に）し、バーの上に "今日の記録は上限（3件）に達しました"（13px, secondary）を表示
...
- **ページ見出しテキストなし**（タブバーのアクティブ状態で現在地が分かるため）。最上部はアイコンボタンの行のみ：
  ...
  - **右上：新しい相談ボタン**（＋アイコンの丸型アウトラインボタン、`aria-label="新しい相談"`）: タップで新しいスレッドを開始（以前のスレッドは履歴に残る）。押下時に「この相談の気づきを記録に残しますか？」の確認を出し、「記録する」で記録タブへ遷移、「そのまま新しい相談」で新スレッドを開く
...
- **スレッド単位のチャットインターフェース**：1つの相談（スレッド）の中では会話を続けられる（追い質問可）
  - 現在のスレッドは保存され、画面を離れて戻ってきても表示される
